...
AREA_CHAR_THRESHOLDS = [(0.20, " "), (0.28, "."), (0.36, ":"), (0.47, "-"), (0.58, "="), (0.70, "+"), (0.82, "*"), (0.93, "#"), (1.01, "%")]


def area_char(level):
  return density_char(level, AREA_CHAR_THRESHOLDS)
  ...
  def render(self, width, height, elapsed, phase, options):
    ...
    for row in range(height):
      ...
      for _, repeat, px in cols:
        ...
        texture = min(1.0, 0.16 + 0.07 * columns_a + 0.05 * stripes + region * (0.38 + 0.24 * stripes + 0.18 * columns_b))
        edge_fade = max(0.0, 1.0 - r2 * r2)
        shadow = 0.62 + 0.38 * math.sqrt(max(0.0, 1.0 - r2))
        level = max(0.0, min(1.0, texture * edge_fade * shadow * options.contrast))
        char = area_char(level)
        color = gray_fg(level)
        if last_color != color:
          line.append(f"\x1b[38;5;{color}m")
          last_color = color
        line.append(char * repeat)
      line.append(RESET)
