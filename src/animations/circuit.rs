...
fn render_small(ctx: &FrameContext, out: &mut String) {
  ...
  for (offset, glyph, speed) in [(0.00, '@', 4.2), (0.33, '*', 3.5), (0.66, '+', 5.0)] {
    for &row in &bus_rows {
      ...
      if col < w { grid[row * w + col] = 1.0; glyphs[row * w + col] = glyph; }
    }
  }
...
impl Animation for Circuit {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    let w = ctx.width; let h = ctx.height;
    if w == 0 || h == 0 { return; }
    if w < 50 || h < 12 { render_small(ctx, out); return; }
    let mut grid = vec![0.0_f64; w * h];
    let mut glyphs = vec![' '; w * h];
    ...
    let hline = |grid: &mut Vec<f64>, glyphs: &mut Vec<char>, row: i64, c0: i64, c1: i64, value: f64, glyph: char| {
      if !(0..h as i64).contains(&row) { return; }
      let (lo, hi) = if c0 < c1 { (c0, c1) } else { (c1, c0) };
      let lo = lo.max(0); let hi = hi.min((w - 1) as i64);
      for c in lo..=hi { put(grid, glyphs, c, row, value, glyph); }
    };
    let vline = |grid: &mut Vec<f64>, glyphs: &mut Vec<char>, col: i64, r0: i64, r1: i64, value: f64, glyph: char| {
      if !(0..w as i64).contains(&col) { return; }
      let (lo, hi) = if r0 < r1 { (r0, r1) } else { (r1, r0) };
      let lo = lo.max(0); let hi = hi.min((h - 1) as i64);
      for r in lo..=hi { put(grid, glyphs, col, r, value, glyph); }
    };
    let bx = |grid: &mut Vec<f64>, glyphs: &mut Vec<char>, c0: i64, r0: i64, ww: i64, hh: i64, label: &str, value: f64| {
      let c1 = (c0 + ww - 1).min((w - 1) as i64);
      let r1 = (r0 + hh - 1).min((h - 1) as i64);
      if c0 >= w as i64 || r0 >= h as i64 { return; }
    ...
    for (idx, &row) in rows.iter().enumerate() {
      hline(&mut grid, &mut glyphs, row, left, right, 0.20 + idx as f64 * 0.025, if idx == 2 { '=' } else { '-' });
    }
    let cols = [left, (w / 4) as i64, mid, ((w * 3) / 4) as i64, right];
    for (idx, &col) in cols.iter().enumerate() {
      vline(&mut grid, &mut glyphs, col, top, bottom, 0.18 + idx as f64 * 0.02, '|');
      for &row in &rows { put(&mut grid, &mut glyphs, col, row, 0.55, '+'); }
    }
    let cpu_b = ((w / 7).max(2) as i64, (h / 3 - 2).max(2) as i64, (w / 7).max(8) as i64, (h / 4).max(5) as i64);
    let mem_b = (((w / 2).saturating_sub(w / 12)).max(2) as i64, (h / 7).max(1) as i64, (w / 6).max(10) as i64, (h / 6).max(4) as i64);
    let io_b = (((w - (w / 8).max(10) - 2).min(w - w / 5)) as i64, (h / 2 - 2).max(2) as i64, (w / 8).max(8) as i64, (h / 4).max(5) as i64);
    let rf_b = ((w / 3).max(2) as i64, (((h - (h / 6).max(4) - 1) as i64).min(bottom - 2)).max(0), (w / 7).max(9) as i64, (h / 6).max(4) as i64);
    ...
    for (path_idx, &(((x0, y0), (x1, y1)), speed, offset, head)) in paths.iter().enumerate() {
      ...
      for tail in 0..10 {
        ...
        if value > 0.25 { put(&mut grid, &mut glyphs, col, row, value, if tail == 0 { head } else { '*' }); }
      }
      if path_idx % 2 == 0 {
