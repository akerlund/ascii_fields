    ...
    for row in 0..h {
      ...
      for &(rep, px) in &cols {
        ...
        let level = if r2 >= 1.0 {
          ...
          let broad_a = smoothstep(
            -0.58,
            0.50,
            sin_y2
              + 0.72 * (1.65 * x - 0.95 * y + 0.50 * t).sin()
              + 0.48 * (2.35 * x + 0.45 * y - 0.25 * t).cos(),
          );
          let broad_b = smoothstep(
            -0.62,
            0.56,
            (1.15 * x + 1.55 * y - 0.68 * t).sin() + 0.54 * (2.05 * x - 0.55 * y + 0.43 * t).cos(),
          );
          let broad_c = smoothstep(
            -0.50,
            0.64,
            (0.85 * x - 1.95 * y + 0.34 * t).cos() + 0.46 * (2.70 * x + 0.25 * y - 0.58 * t).sin(),
          );
          ...
          let texture =
            (0.16 + 0.07 * columns_a + 0.05 * stripes + region * (0.38 + 0.24 * stripes + 0.18 * columns_b))
              .min(1.0);
