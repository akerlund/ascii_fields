...
impl Animation for Network {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    if w == 0 || h == 0 { return; }
    let dw = (w - 1) as f64; let dh = (h - 1) as f64;
    ...
    for (edge_idx, &(a, b)) in EDGES.iter().enumerate() {
      let (c0, r0) = point(a); let (c1, r1) = point(b);
      let dx = (c1 - c0) as f64; let dy = (r1 - r0) as f64;
    ...
    for (idx, &(_, _, kind)) in NODES.iter().enumerate() {
      ...
      match kind {
        ...
        'E' => { put(&mut grid, &mut glyphs, col, row, 0.78 + 0.20 * beat, 'O'); }
        _ => {
