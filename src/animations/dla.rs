...
impl Dla {
  ...
  fn walk_one(&mut self) {
    ...
    for _ in 0..20 {
      ...
      if !self.cluster[(y as usize) * w + (x as usize)] { break; }
    }
    for _ in 0..MAX_WALK {
...
impl Animation for Dla {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    if size >= cap { self.seed(ctx.width, ctx.height); }
    else { for _ in 0..walkers { self.walk_one(); } }
    ...
    for r in 0..ctx.height {
      ...
      for c in 0..ctx.width {
        ...
        if a > 0 { grid[base + c] = clamp((0.22 + 0.78 * a as f64 / max_age) * contrast); }
      }
    }
