...
fn orb_1s(_x: f64, _z: f64, r: f64) -> f64 { (-r).exp() }
fn orb_2s(_x: f64, _z: f64, r: f64) -> f64 { (1.0 - 0.5 * r) * (-0.5 * r).exp() }
fn orb_2pz(_x: f64, z: f64, r: f64) -> f64 { z * (-0.5 * r).exp() }
fn orb_3dz2(_x: f64, z: f64, r: f64) -> f64 { (3.0 * z * z - r * r) * (-r / 3.0).exp() }
fn orb_3dxz(x: f64, z: f64, r: f64) -> f64 { x * z * (-r / 3.0).exp() }
fn orb_4fz3(_x: f64, z: f64, r: f64) -> f64 { z * (5.0 * z * z - 3.0 * r * r) * (-0.25 * r).exp() }

pub struct Orbitals { norm: HashMap<usize, f64> }
impl Default for Orbitals { fn default() -> Self { Self { norm: HashMap::new() } } }

impl Orbitals {
  fn normalizer(&mut self, idx: usize) -> f64 {
    if let Some(&v) = self.norm.get(&idx) { return v; }
...
impl Animation for Orbitals {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    let n0 = self.normalizer(i0); let n1 = self.normalizer(i1);
    ...
    for row in 0..h {
      ...
      for col in 0..w {
        ...
        let x0 = px * ext0; let z0 = pz * ext0;
        let a0 = f0(x0, z0, (x0 * x0 + z0 * z0).sqrt());
        let d0 = a0 * a0 * n0;
        let d = if blend > 0.0 {
          let x1 = px * ext1; let z1 = pz * ext1;
