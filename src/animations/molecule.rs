...
fn build_bonds(atoms: &[(f64, f64, f64)]) -> Vec<(usize, usize)> {
  ...
  for i in 0..n {
    ...
    for &(j, _) in &dists[1..4] {
      ...
      if !bonds.contains(&edge) { bonds.push(edge); }
    }
  }
  bonds
}

pub struct Molecule { atoms: Vec<(f64, f64, f64)>, bonds: Vec<(usize, usize)> }
...
impl Animation for Molecule {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    let ay = ctx.elapsed * 0.6; let ax = ctx.elapsed * 0.37;
    ...
    let put = |grid: &mut Vec<f64>, col: f64, row: f64, val: f64| {
      let c = col.round() as i64; let r = row.round() as i64;
    ...
    for &(i, j) in &self.bonds {
      let (x0, y0, z0) = pts[i]; let (x1, y1, z1) = pts[j];
