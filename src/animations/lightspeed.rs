...
const TH: &[(f64, char)] =
  &[(0.05, ' '), (0.20, '.'), (0.40, ':'), (0.58, '-'), (0.72, '+'), (0.84, '*'), (0.93, '#'), (1.01, '@')];
const CYCLE: f64 = 9.0;
...
impl Default for Lightspeed {
  fn default() -> Self {
    Self { stars: Vec::new(), rng: Pcg32::from_entropy() }
  }
}
...
impl Animation for Lightspeed {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    let n = 60.max(w * h / 18);
    if self.stars.len() != n {
      self.stars = (0..n).map(|_| (self.rng.gen_range(0.0..TAU), self.rng.gen_range(0.0..1.0))).collect();
    }
    ...
    for &(ang, r0) in &self.stars {
      ...
      let x0 = cx + ca * r;
      let y0 = cy + sa * r;
      let x1 = cx + ca * (r + length);
      let y1 = cy + sa * (r + length);
