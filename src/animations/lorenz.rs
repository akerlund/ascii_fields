...
impl Default for Lorenz {
  fn default() -> Self { Self { w: 0, h: 0, grid: Vec::new(), x: 0.1, y: 0.0, z: 0.0, last: 0.0 } }
}
...
impl Animation for Lorenz {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    for v in self.grid.iter_mut() { *v *= decay; }
    ...
    let mut x = self.x; let mut y = self.y; let mut z = self.z;
    for _ in 0..steps.max(1) {
      let dx = SIGMA * (y - x);
      let dy = x * (RHO - z) - y;
      let dz = x * y - BETA * z;
      x += dx * DT; y += dy * DT; z += dz * DT;
      let ci = (cx + x * fx) as i64; let ri = (cy + (z - 25.0) * fy) as i64;
      if ci >= 0 && (ci as usize) < ctx.width && ri >= 0 && (ri as usize) < ctx.height {
        let idx = (ri as usize) * ctx.width + (ci as usize);
        if self.grid[idx] < 1.0 { self.grid[idx] = (self.grid[idx] + 0.55).min(1.0); }
      }
    }
    self.x = x; self.y = y; self.z = z;
    let contrast = ctx.options.contrast;
    let mut out_grid = vec![0.0_f64; ctx.width * ctx.height];
    for i in 0..out_grid.len() { out_grid[i] = clamp(self.grid[i] * contrast); }
