...
#[derive(Clone, Copy)] enum Kind { Fermion, Photon, Gluon }
enum Element {
  Vertex(f64, f64),
...
fn diagrams() -> Vec<Vec<Element>> {
  ...
  vec![
    vec![
      Label(0.02, 0.18, "e-"), Label(0.02, 0.82, "e+"),
      ...
      Label(0.95, 0.18, "u-"), Label(0.95, 0.82, "u+"),
    ],
    vec![
      Label(0.02, 0.30, "y"), Label(0.02, 0.86, "e-"),
...
pub struct Feynman { diagrams: Vec<Vec<Element>> }
impl Default for Feynman { fn default() -> Self { Self { diagrams: diagrams() } } }
...
impl Animation for Feynman {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    let put = |grid: &mut Vec<f64>, glyphs: &mut Vec<char>, x: f64, y: f64, value: f64, ch: char| {
      let c = x.round() as i64; let r = y.round() as i64;
    ...
    for el in &self.diagrams[didx] {
      match *el {
        Element::Vertex(x, y) => { put(&mut grid, &mut glyphs, x * dw, y * dh, 1.0, '@'); }
        ...
        Element::Segment(kind, x0, y0, x1, y1) => {
          let cx0 = x0 * dw; let cy0 = y0 * dh;
          let cx1 = x1 * dw; let cy1 = y1 * dh;
          let dx = cx1 - cx0; let dy = cy1 - cy0;
          let length = (dx * dx + dy * dy).sqrt().max(1.0);
          let steps = (length as i64).max(2);
          let perp_x = -dy / length; let perp_y = dx / length;
          ...
          for s in 0..=steps {
            ...
            let ch = match kind {
              Kind::Photon => {
                ...
                x += perp_x * wig; y += perp_y * wig; '~'
              }
              Kind::Gluon => {
                let wig = 1.6 * (s as f64 * 1.0).sin();
                x += perp_x * wig; y += perp_y * wig; 'o'
              }
