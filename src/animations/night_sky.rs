...
impl Animation for NightSky {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    let ca = band_angle.cos(); let sa = band_angle.sin();
...
fn meteor_brightness(m: &Meteor, col: f64, row: f64) -> f64 {
  ...
  let ux = m.dx / length; let uy = m.dy / length;
  let rx = col - m.head_x; let ry = row - m.head_y;
  let along = rx * ux + ry * uy;
  let perp = (rx * uy - ry * ux).abs();
  if along > 0.6 || along < -10.0 || perp > 1.4 { return 0.0; }
