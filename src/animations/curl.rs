...
impl Curl {
  fn seed(&mut self, w: usize, h: usize) {
    self.w = w; self.h = h;
    let n = 80.max(w * h / 28);
    self.particles = (0..n).map(|_| (
      self.rng.gen_range(0.0..w as f64),
      self.rng.gen_range(0.0..h as f64),
    )).collect();
...
impl Animation for Curl {
  fn render(&mut self, ctx: &FrameContext, out: &mut String) {
    ...
    for v in self.trail.iter_mut() { *v *= TRAIL_DECAY; }
    ...
    for &(x, y) in &self.particles {
      ...
      if nx_pos < 0.0 { nx_pos += w as f64; }
      if ny_pos < 0.0 { ny_pos += h as f64; }
      let ci = nx_pos as usize;
      let ri = ny_pos as usize;
      if ci < w && ri < h {
        let idx = ri * w + ci;
        if self.trail[idx] < 1.0 { self.trail[idx] = (self.trail[idx] + 0.5).min(1.0); }
      }
    ...
    for i in 0..grid.len() { grid[i] = clamp(self.trail[i] * contrast); }
