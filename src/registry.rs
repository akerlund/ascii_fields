...
/// Returns true if the given mode actually reads `RenderOptions::scale` in
/// its render function. Used by the HUD to display "N/A" for the scale
/// slot on modes that ignore it.
///
/// Generated by `grep -l 'options.scale' src/animations/` -- keep this in
/// sync with the source. Modes not in this list silently ignore --scale.
pub fn supports_scale(name: &str) -> bool {
  matches!(
    name,
    "areas"
      | "black-hole"
