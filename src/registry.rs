...
pub static MODES: &[ModeInfo] = &[
  ...
  ModeInfo { name: "caustics",       description: "Underwater light caustics rippling over a surface",      factory: || Box::new(terrain::Caustics::default()) },
  ...
  ModeInfo { name: "penrose",        description: "Aperiodic Penrose-like interference tiling",             factory: || Box::new(patterns::Penrose::default()) },
  ...
  ModeInfo { name: "pulsar",         description: "Rotating neutron-star beam sweep",                       factory: || Box::new(space::Pulsar::default()) },
  ModeInfo { name: "supernova",      description: "Expanding stellar shock shell and filaments",            factory: || Box::new(space::Supernova::default()) },
  ModeInfo { name: "solar-wind",     description: "Charged particles flowing around a magnetosphere",       factory: || Box::new(space::SolarWind::default()) },
  ...
  ModeInfo { name: "ferrofluid",     description: "Magnetic fluid spikes around moving field sources",      factory: || Box::new(physics::Ferrofluid::default()) },
  ...
  ModeInfo { name: "schlieren",      description: "Heat-haze and shockwave density gradients",              factory: || Box::new(physics::Schlieren::default()) },
  ModeInfo { name: "seismograph",    description: "Earthquake wavefronts through layered ground",           factory: || Box::new(terrain::Seismograph::default()) },
  ModeInfo { name: "convection",     description: "Rayleigh-Benard-like heat convection rolls",             factory: || Box::new(physics::Convection::default()) },
  ModeInfo { name: "reconnection",   description: "Magnetic field lines snapping and reconnecting",         factory: || Box::new(physics::Reconnection::default()) },
  ...
  ModeInfo { name: "topography",     description: "Animated contour map with rivers and flow lines",        factory: || Box::new(terrain::Topography::default()) },
  ModeInfo { name: "dunes",          description: "Wind-driven sand ripples migrating over dunes",          factory: || Box::new(terrain::Dunes::default()) },
  ModeInfo { name: "phyllotaxis",    description: "Golden-angle sunflower spiral",                          factory: || Box::new(phyllotaxis::Phyllotaxis) },
  ModeInfo { name: "quasicrystal",   description: "Fivefold wave interference quasicrystal",                factory: || Box::new(patterns::Quasicrystal::default()) },
  ModeInfo { name: "moire",          description: "Rotating line-grid moire interference",                  factory: || Box::new(patterns::Moire::default()) },
  ModeInfo { name: "voronoi",        description: "Moving Voronoi cell boundaries",                         factory: || Box::new(patterns::Voronoi::default()) },
  ModeInfo { name: "reaction-rings", description: "Belousov-Zhabotinsky-style chemical wave rings",        factory: || Box::new(patterns::ReactionRings::default()) },
  ModeInfo { name: "nbody",          description: "Gravitational bodies orbiting through a shared field",   factory: || Box::new(space::NBody::default()) },
  ModeInfo { name: "strange",        description: "Morphing De Jong strange attractor",                     factory: || Box::new(patterns::Strange::default()) },
